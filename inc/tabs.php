...
class dciSkin_tabs
{
    ...
    public static function getRootCourse($current_ref_id)
    {
        ...
        $parent_node_tree = ["ref_id" => $current_ref_id];
        for ($ref_id = $current_ref_id; $ref_id; $parent_node_tree = $tree->getParentNodeData($ref_id)) {
            $ref_id = isset($parent_node_tree['ref_id']) ? $parent_node_tree['ref_id'] : false;
            if (!empty($ref_id)) {
    ...
    public static function getCourseTabs($ref_id = null)
    {
        ...
        $from_cache = dciSkin_cache::get('dciSkin_tabs::getCourseTabs', $root_course['ref_id'] ?? 0);
        if (dciSkin_cache::is_valid($from_cache)) {
            return $from_cache;
        }
            
        $tabs = static::getChildArray($root_course['ref_id'] ?? 0, $current_ref_id);
    ...
    {
        $from_cache = dciSkin_cache::get('dciSkin_tabs::getCardsOnPage', $obj_id);
        if (dciSkin_cache::is_valid($from_cache)) {
            return $from_cache;
        }
        ...
        $page_content = $db->fetchAssoc($res)["content"] ?? null;
        ...
        if (empty($page_content)) {
            dciSkin_cache::add('dciSkin_tabs::getCardsOnPage', $ids, $obj_id);
            return $ids;
        }
    ...
    {
        $from_cache = dciSkin_cache::get('dciSkin_tabs::getH1', $obj_id);
        if (dciSkin_cache::is_valid($from_cache)) {
            return $from_cache;
        }
        ...
        if (empty($page_content)) {
            ...
            $page_content = $db->fetchAssoc($res)["content"] ?? null;
            if (empty($page_content)) {
                dciSkin_cache::add('dciSkin_tabs::getH1', "", $obj_id);
                return "";
            }
        }
        ...
        if (! empty($first_h1)) {
            dciSkin_cache::add('dciSkin_tabs::getH1', $first_h1->textContent, $obj_id);
